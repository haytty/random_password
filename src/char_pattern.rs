    ...
    R: Rng + ?Sized,
{
    ...
    let mut char_pattern_length_map: HashMap<CharPattern, Length> = HashMap::new();
    for (&pattern, &length) in patterns.iter().zip(random_length_vec.iter()) {
        char_pattern_length_map.insert(pattern, length);
    }
